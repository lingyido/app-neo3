...
void script_hash_to_address(char* out, size_t out_len, const unsigned char* script_hash) {
    ...
    CX_CHECK(cx_hash_no_throw(&data_hash.header, CX_LAST, address, UINT160_LEN + 1, data_hash_1, 32));
    cx_sha256_init(&data_hash);
    CX_CHECK(cx_hash_no_throw(&data_hash.header, CX_LAST, data_hash_1, SHA256_HASH_LEN, data_hash_2, 32));
